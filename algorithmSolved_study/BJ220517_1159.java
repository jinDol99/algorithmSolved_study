		...
		for (int i = 97; i < 123; i++) {
			for (int j = 0; j < mem.length; j++) {
				if (mem[j].charAt(0) == i) {
					cnt++;
					if (cnt == 5) {
						System.out.print(mem[j].charAt(0));
						say++;
						break;
					}
				}	
...
각 선수의 능력도 알지 못한다. 따라서, 누가 선발인지 기억하기 쉽게 하기 위해 성의 첫 글자가 
같은 선수 5명을 선발하려고 한다. 만약, 성의 첫 글자가 같은 선수가 5명보다 적다면, 
